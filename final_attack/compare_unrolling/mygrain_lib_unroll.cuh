...
__device__ void init_grain_unroll_1(grain_state *grain, uint8_t *key, uint8_t *iv){
	// expand the packed bytes and place one bit per array cell (like a flip flop in HW)
	for (int i = 0; i < 12; i+=4) {
		grain->lfsr[8 * i] = (iv[i] & (1 <<7)) >> 7;
		grain->lfsr[8 * i + 1] = (iv[i] & (1 << 6)) >> 6;
...
__device__ uint8_t encrypt_unroll(uint8_t *iv, uint8_t *key){
