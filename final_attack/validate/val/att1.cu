...
__global__ void key_gen_superpoly_reconstruction(uint8_t *k_set,uint8_t len){
	uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
	uint8_t *k_start, pos = 127-tid;
	if(tid<len){
		/*
			c_i = ((tid/(1ull<<i)))%2;
...
__global__ void cuda_encrypt_constant_key(uint8_t *cube,uint8_t *out){
	uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
	uint8_t IV[IV_dim];
	IV_gen(tid,k_curr_dev,cube,IV);
	out[tid] = encrypt_unroll(IV,d_k);
}*/

__global__ void cuda_encrypt_2_exploit(uint8_t *cube,uint8_t *out){
	uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
	uint8_t IV[IV_dim];
	IV_gen(tid,k_curr_dev,cube,IV);
	out[tid] = encrypt_unroll_exploit(IV);
}
...
uint8_t check_p_coeff_1(uint8_t *IV,uint64_t len){
	uint8_t gpu_sum=0,c;
	for(int i=0;i<len;i++){
		c = encrypt_host_exploit(&IV[i*IV_dim]);
		gpu_sum ^= c;
	}
	return gpu_sum;
}
...
int main(int argc, char *argv[]){
	...
	print_cubes(cs);
	printf("\nOK\n");
	uint8_t *out_cube,*reduce_out,*dev_reduce_out_n,*hreduce_out_n=NULL,*dev_out_cube_n;
	...
	CHECK(cudaMemcpyToSymbol(d_key_2_guess, h_key_2_guess, K_dim*sizeof(uint8_t), 0, cudaMemcpyHostToDevice));
	...
	for(int c=0;c<cs.n_cubes;c++){
		...
		cuda_encrypt_2_exploit<<<grid,block>>>(&dev_I[start_I],dev_out_cube_n);
		CHECK(cudaDeviceSynchronize());
	...
	/*
    FILE *fp = fopen("./final_attack/offline/superpolies_right_1.txt","w");
