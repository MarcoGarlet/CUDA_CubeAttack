		...
		for(int c=0; c<cs.n_cubes; c++){
			...
			for(int i=0;i<N*CONSTANT;i++){
				...
				if(i==0){
					...
					p0_host = compute_cube_host(k0,&cs.cubes[start_I],data_len,cs.cubes_len[c]);
					n1+=(p0==1);
					n0+=(p0==0);
				}
				...
				p1_host = compute_cube_host(k1,&cs.cubes[start_I],data_len,cs.cubes_len[c]);
				p2_host = compute_cube_host(k2,&cs.cubes[start_I],data_len,cs.cubes_len[c]);
				p1_2_host = compute_cube_host(kx,&cs.cubes[start_I],data_len,cs.cubes_len[c]);
				...
				/*Linearity test*/
				if (p0_host ^ p1_host ^ p2_host != p1_2_host){
