...
__global__ void cuda_encrypt(uint8_t *I,uint8_t *out,uint64_t window){
	uint8_t IV[IV_dim];
	uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
	uint8_t pr = 0;
	for(uint64_t i=0;i<window;i++){
		IV_gen(((tid)*window)+i,k_curr_dev,I,IV); 
...
uint64_t get_nbytes_IV_single(uint8_t ki){
	return 1<<ki;
} 
...
uint8_t compute_cube(uint8_t *k,uint8_t *dev_I,cudaStream_t si,uint64_t length){

	uint8_t *hreduce_out,gpu_sum,*out_cube,*reduce_out;
	uint64_t blocksize;

	// check blocksize for given length, the length is 2**k

	
	if(length/(1<<24) > 0) // 2*25
		blocksize = 1024;
	else	
		blocksize=32;

	uint64_t window = ((length/(1<<30))>0)? length/(1<<30):1;
	length/=window;
	...
	//CHECK(cudaMallocAsync((void **)&out_cube,length*(sizeof(uint8_t)),si));
	CHECK(cudaMalloc((void **)&out_cube,length*(sizeof(uint8_t))));
	//CHECK(cudaStreamSynchronize(si));
	//CHECK(cudaMallocAsync((void **)&reduce_out,length*(sizeof(uint8_t)),si));
	CHECK(cudaMalloc((void **)&reduce_out,length*(sizeof(uint8_t))));

	//CHECK(cudaStreamSynchronize(si));
	cudaMemcpyToSymbolAsync(d_k,k, sizeof(uint8_t)*K_dim,0,cudaMemcpyHostToDevice,si);
	CHECK(cudaStreamSynchronize(si));
	printf("\nCUDA, length = %lu, cuda_encrypt <<<%u,%u>>> window = %lu\n",length,grid.x,block.x,window);

	cuda_encrypt<<<grid,block,0,si>>>(dev_I,out_cube,window);
	CHECK(cudaStreamSynchronize(si));
...
int main(){
	...
	uint64_t del_1[INSTANCES],add_1[INSTANCES],ACTIVE_INSTANCES=INSTANCES,n0,n1;
	...
	uint8_t *maxterms=NULL,*out_I,*out_I_host=h_IV,*out_k_host;
	...
	for(int c=0; c<ACTIVE_INSTANCES; c++){
		...
		for(int i=0;i<N*CONSTANT;i++){
			...
			//print_arr_host("K2: ",k2,K_dim);
				...
				if(i==0){
					p0 = compute_cube(k0,&out_I[start_I],stream[i],data_len);
				...
				p1 = compute_cube(k1,&out_I[start_I],stream[i],data_len);
				p2 = compute_cube(k2,&out_I[start_I],stream[i],data_len);
				p1_2 = compute_cube(kx,&out_I[start_I],stream[i],data_len);
