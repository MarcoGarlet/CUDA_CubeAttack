...
int main(int argc, char *argv[]){
	...
	uint8_t *k_set,*k_set_host,*out_cube,*reduce_out,*dev_reduce_out_n,*hreduce_out_n=NULL,*dev_out_cube_n;
	...
	uint64_t window,data_len_d;
	for(int c=0;c<cs.n_cubes;c++){

		data_len = (uint64_t)1<<(uint64_t)cs.cubes_len[c];
		data_len_d=data_len;
		if(data_len_d/(1<<24) > 0) // 2*25
			blocksize = 1024;
		else	
			blocksize=32;

		window = ((data_len_d/(1<<30))>0)? data_len_d/(1<<30):1;
		//window = 2;
		data_len_d/=window;
		blocksize = (data_len_d<blocksize)? data_len_d:blocksize;

		block.x = blocksize;
		grid.x = (data_len_d+ block.x - 1) / block.x;
		...
		// compute for k0
		CHECK(cudaMalloc((void **)&dev_out_cube_n,data_len_d*sizeof(uint8_t)));
		CHECK(cudaMalloc((void **)&dev_reduce_out_n,data_len_d*sizeof(uint8_t)));
		...
		CHECK(cudaDeviceSynchronize());
		sumZ2CubeReduceInterleaved<<<grid,block>>>(dev_out_cube_n,dev_reduce_out_n,data_len_d);
		CHECK(cudaDeviceSynchronize());
