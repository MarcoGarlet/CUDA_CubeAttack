...
uint8_t compute_cube(uint8_t *k,uint8_t *dev_I,cudaStream_t si,uint64_t length){
	...
	if(length/(1ull<<24) > 0) // 2*25
		blocksize = 1024;
	else	
		blocksize=32;

	uint64_t window = ((length/(1ull<<30))>0)? length/(1ull<<30):1;
	length/=window;
	...
	CHECK(cudaStreamSynchronize(si));
	//printf("\nCUDA, length = %llu, cuda_encrypt <<<%llu,%llu>>> window = %llu\n",length,grid.x,block.x,window);
...
int main(){
	...
	uint8_t **updated_cubes,*maxterms=NULL;
