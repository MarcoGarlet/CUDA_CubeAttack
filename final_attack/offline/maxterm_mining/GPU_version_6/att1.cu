...
int main(int argc, char *argv[]){
	...
	*/
		...
		*/
		...
		for(int c=0; c<ACTIVE_INSTANCES; c++){
			...
			for(unsigned int i=0;i<N*CONSTANT;i++){
				//print_arr_host("OUT_I p1 check",&out_I_host[start_I],out_k_host[c]);
				//print_arr_host("K",&k1_set_host[i*K_dim],K_dim);
				//printf("P1[%d] = %d, check = %d",i,p1_set[i],check_p_coeff(&k1_set_host[i*K_dim],&out_I_host[start_I],1<<out_k_host[c],out_k_host[c]));
    ...
    printf("\n############ SOLUTIONS ############\n");
    FILE *fp = fopen("./final_attack/offline/cubes_test_window.txt","a");
